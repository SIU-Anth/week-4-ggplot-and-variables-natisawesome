...
```{r}
ggplot(data = df, aes(x = hemoglobin)) + 
  geom_histogram(bins = 30, fill = 'steelblue', color = "white") 

```

```{r}
I chose to do the above histogram because it shows the distribution of the values. A future researcher could analyze this a discuss the skew and the reason it is skewed.
```


I also tried to make a scatterplot, but alas it appears to just be lines.
...
```{r}
I chose to do the below histogram because it shows the distribution of values and it is color coded. This graph helps provide a more in depth look at how the data is distributed between groups.
```


```{r}
ggplot(data = df, aes(y = hemoglobin, x = group, color = group)) +
...
The dependent variable is the hemoglobin levels. This is because the hemoglobin levels are what is being measured. The independent variables are group and sex. This is because they are being recorded to analyze the effect they may have on the dependent variable.
...
Groups from higher altitudes appear to have higher hemoglobin levels. This may mean that to process the oxygen in high altitudes their bodies adapted to be able to produce more hemoglobin.In the future, researches could look at diet and activity to see if those variables have a role on hemoglobin counts in high altitude. 
